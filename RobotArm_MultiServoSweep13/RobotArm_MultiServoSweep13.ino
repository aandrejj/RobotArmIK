...
/*
rampInt  myAngle01;                       // create a byte ramp object to interpolate servo position
rampInt  myAngle02;
rampInt  myAngle03;
rampInt  myAngle04;
rampInt  myAngle05;
rampInt  myAngle06;
*/
rampInt myAngles[7];// create a byte ramp object to interpolate servos position
byte  previousAngles[7] =[0,0,0,0,0,0,0,0];

/*
byte  previousAngle01 = 0;
byte  previousAngle02 = 0;
byte  previousAngle03 = 0;
byte  previousAngle04 = 0;
byte  previousAngle05 = 0;
byte  previousAngle06 = 0;
*/
...
void mainRoboArmUpdate() {
  updateServos();   // update servos according to InverseKinematics Values
  delay(10);       // lazy way to limit update to 100 Hz
}
//--------------------End of mainRoboArmUpdate----------------------

//--------------------updateServo()--------------------------------
void updateServo(int servoNumber) {
  int val = myAngle[servoNumber].update();           // update ramp value
  Serial.print("updateServo: servoNumber = ");
  Serial.print(servoNumber);
  Serial.print((" , myAngle = ")
  Serial.print(myAngle[servoNumber].getValue());   // prompt angle value
  myservo[servoNumber].write(myAngle[servoNumber].getValue());    // transmit it to the servo
}
 //--------------------end of updateServo-----------------------------

//--------------------updateServos()--------------------------------
void updateServos() {
  for (int servoNumer = 1; servoNumer <= 6; servoNumer++) {
    updateServo( servoNumber);
  }
}
 //--------------------end of updateServo-----------------------------
